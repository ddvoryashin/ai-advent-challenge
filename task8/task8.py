...
@dataclass
class TokenStats:
    request_tokens: int   # tokens sent in current request (from API)
    response_tokens: int  # tokens in model reply (from API)
    history_tokens: int   # estimated tokens for full conversation history
...
class Agent:
    ...
    def chat(self, user_message: str) -> tuple[str, int, TokenStats]:
        """Returns (reply, messages_excluded_from_window, token_stats)."""
        ...
        total_msgs, history_tokens = history_stats(self._history_file, self._system)
        excluded = max(0, total_msgs - len(messages))
        ...
        return reply, excluded, token_stats
...
def main() -> None:
    ...
    while True:
        ...
        reply, excluded, tok = agent.chat(user_input)
        print(f"Agent: {reply}")
        print(
            f"[tokens — request: {tok.request_tokens} | response: {tok.response_tokens} | "
            f"full history: ~{tok.history_tokens}"
            + (f" | {excluded} old msg(s) outside window" if excluded else "")
            + "]\n"
        )
